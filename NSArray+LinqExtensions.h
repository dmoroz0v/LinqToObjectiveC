...
typedef BOOL (^LINQCondition)(ObjectType item);

typedef id (^LINQSelector)(ObjectType item);

typedef id (^LINQAccumulator)(ObjectType item, id aggregate);
 ...
 */
- (NSArray<ObjectType> *)linq_where:(LINQCondition)predicate;
...
/** Sorts the elements of a sequence in ascending order by using a specified keySelector.
 ...
 @return An array whose elements are sorted in ascending order.
 */
- (NSArray<ObjectType> *)linq_sort:(LINQSelector)keySelector;
...
/** Returns distinct elements from a sequence, where the given selector is used to specify the value to use for equality for each item.
 ...
 @return An array of distinct elements.
 */
- (NSArray<ObjectType> *)linq_distinct:(LINQSelector)keySelector;
 ...
 */
- (ObjectType)linq_firstOrNil:(LINQCondition)predicate;
 ...
 */
- (NSDictionary<id, NSArray<ObjectType> *> *)linq_groupBy:(LINQSelector)groupKeySelector;
